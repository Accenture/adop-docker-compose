...
docker-machine ssh $MACHINE_NAME "mkdir -p /home/ubuntu/volumes; sudo mkdir -p $DIR/volumes"
docker-machine scp -r /root/docker-compose/volumes/nginx-configuration-vol/ $MACHINE_NAME:/home/ubuntu/volumes/
docker-machine scp -r /root/docker-compose/volumes/nginx-releasenote-vol/ $MACHINE_NAME:/home/ubuntu/volumes/
...
docker exec dockercompose_ldap_1 bash -c '/usr/local/bin/load_ldif.sh -h $(hostname) -u cn=admin,dc=adop,dc=accenture,dc=com -p Sw4syJSWQRx2AK6KE3vbhpmL -b dc=adop,dc=accenture,dc=com -f /tmp/structure.ldif'
